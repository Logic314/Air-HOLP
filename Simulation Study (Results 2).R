...
# This file conducts the spatial correlation simulation study in section 4 and 
# saves the results in the files "Threshold results 2.csv" and
...
# Simulation settings
r0 <- 600 # Penalty parameter of Ridge-HOLP
...
if(Figure == "1a"){
  ...
  #N <- c(125,250); P <- 250; P0 <- c(9,12); Rsq <- c(.9,0.95)
}
if(Figure == "1b"){
...
for (bx in 1:BX) {
  ...
  for (j in 1:length(N)) {
    ...
    for (k in 1:length(P)) {
      ...
      Xsub <- matrix(rmvn(n*p*4/5, 0, 1, ncores = nc), nrow = n, ncol = p*4/5)
      Xsub <- scale(Xsub)
      Z <- matrix(rmvn(n*(p/5+1), 0, 1, ncores = nc), nrow = n, ncol = p/5+1)
      Z <- scale(Z)
      # Calculations on the independent features:
      ZZT <- tcrossprod(Z[,1:(p/5)])
      Z1zT <- rowSums(Z[,1:(p/5)])%*%t(Z[,p/5+1])
      zzT <- tcrossprod(Z[,p/5+1])
      for (i in 1:length(Rho)) {
        rho <- Rho[i]
        # Generating the features matrix X1 and XXT
        X1 <- sqrt(1 - rho)*Z[,1:(p/5)] +
          sqrt(rho)*kronecker(matrix(1,1,p/5),Z[,p/5+1])
        XXT1 <- (1-rho)*ZZT + sqrt(rho*(1-rho))*(Z1zT + t(Z1zT)) + rho*p*zzT
        XXTsub <- tcrossprod(Xsub)
        XXT <- XXTsub + XXT1
        X <- cbind(Xsub[,1:(p*2/5)], X1, Xsub[,(p*2/5+1):(p*4/5)])
        # Calculations on X:
        eXXT <- eigen(XXT)
        Lambda <- eXXT$values
        U <- eXXT$vectors
        ...
        for (by in 1:By) {
          ...
          for (l in 1:length(P0)) {
            p0 <- P0[l]
            ids <- sample(1:p)[1:p0] # indexes of true features
            rho_ids <- (ids > 0.4*p)*(ids <= 0.6*p)
            X0 <- X[,ids] # matrix of true features
            Beta0 <- 2*((runif(p0)>0.4)-0.5)*(abs(rnorm(p0))+4*log(n)/sqrt(n))
            y0 <- X0%*%Beta0 # expected response vector
            XTy0 <- crossprod(X,y0)
            for (m in 1:length(Rsq)) {
              y0_var <- (1 - rho)*crossprod(Beta0[rho_ids]) +
                rho*(sum(Beta0[rho_ids]))^2 + crossprod(Beta0[1 - rho_ids])
              ...
              Thr_SIS[(bx-1)*By+by,i,j,k,l,m] <- max(rank_SIS[ids])
              ...
              Thr_r[(bx-1)*By+by,i,j,k,l,m] <- max(rank_r[ids])
              Thr_r0[(bx-1)*By+by,i,j,k,l,m] <- max(rank_r0[ids])
            }
          }
        }
      }
...
write.csv(Threshold_results, "Threshold results 2.csv")
...
write.csv(Probability_results, "Probability results 2.csv")
if(Figure != "All"){
